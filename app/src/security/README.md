...
The first scope is canonicalized to unique registered names and the registered called skill is always included. An unknown called skill receives one constrained planner retry. A later attempt to change the frozen scope still terminates before any tool runs, and the executor retains its own request-scope denial as defense in depth. Invalid planner JSON/shape is retried once. If repair fails before any tool execution, Shiva falls back to its grounded core chat path with no claim of tool or live-source use; once a tool observation exists, planner failures remain fail-closed. All skill arguments pass through strict Zod schemas. Built-in contracts remain registered even when Google Sheets or Brave is not configured, but execution returns a safe unavailable observation without contacting the missing integration. Tool failures become sanitized observations. The agent loop rejects a success response until every selected skill has at least one `success=true` observation; a failure response needs a failed selected-skill observation. Tool-free conversation is delegated to the existing provider stream, capability answers are generated from the registry rather than model folklore, and clarification cannot claim execution. Every run remains bounded by `AGENT_MAX_STEPS` and one shared `AGENT_REQUEST_TIMEOUT_MS` deadline.
