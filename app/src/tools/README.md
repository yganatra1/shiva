...
There is no keyword intent router. On every non-explicit-memory turn the planner first sees the pack catalog and selects `direct_chat`, a capability summary, a clarification, `open_packs`, or a skill call. The first skill call freezes the opened pack set, not an individual tool list, so any skill in those packs remains available for a discovered multi-step chain. A direct first call freezes every pack represented by its validated `selectedSkills`. The executor still receives concrete allowed skill names as defense in depth, validates input, resolves runtime-owned action metadata, and applies the global execution policy before a tool runs. Tool results become structured observations. The planner's `respond` decision contains only the grounded user-facing message; runtime observations own execution success/failure, including successful empty searches. A common matching skill-name/type discriminator slip is repaired locally and other malformed decisions are retried once. After a pack has opened, a second malformed result returns “no action was executed” directly and cannot fall through to normal chat. All planner and tool steps share one `AGENT_REQUEST_TIMEOUT_MS` deadline.
