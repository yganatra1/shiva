...
function buildRules(
  ...
  if (role === "core") {
    rules.push(
      ...
      "- Before the first skill call of a compound delegated request, resolve any minimal context Core needs itself (for example a person via person_search), so it can send only the required contact details to the specialized agent.",
      "- Name lookups (person_search, or a device contact search relayed back by an agent) match loosely and can return more than one plausible person for an ambiguous or common name. If more than one candidate is plausible, do not guess which one is meant. List the candidates with a distinguishing detail (relationship, phone, etc.) and return a respond decision asking the user to pick one, before placing a call, sending a message, or otherwise acting on their contact details.",
    );
  }
