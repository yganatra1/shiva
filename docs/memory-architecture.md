...
For each meaningful request — a live chat message or a scheduled task's stored instruction firing, both of which flow through `ShivaChatService.startResponseTo` — Shiva embeds the query, retrieves active semantic and episodic candidates by cosine similarity, reranks them using 70% similarity, 20% importance, and 10% recency, and injects only the top configured results. This same retrieved context reaches both the direct-chat response and Core's execution planner (`AgentRequest.relevantMemoryContext`), so every planner decision always has it available without needing to call `memory_search` first; that skill remains for a deeper, on-demand lookup beyond what's already surfaced. Retrieved records have their access timestamp and count updated.
