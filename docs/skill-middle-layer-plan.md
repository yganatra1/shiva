...
**Superseded routing note (2026-08-23):** those five high-level adapters are no longer registered in the main Shiva runtime. Every Android request now enters through the sensitive `delegate_to_agent` skill and the `device` agent owns selection and execution of all 17 `device.*` commands, including single-step reads. The old adapters remain only as compatibility/test components; see `docs/device-architecture.md` for the current flow.

**Historical direct-adapter implementation:** `device_camera_capture` added the `ChatMessage.images` field and performed a separate vision description call in the main process. Under the superseding delegated flow, the device agent instead attaches camera and screenshot bytes to its own planner call while redacting them from textual history and traces. The old adapter remains only for compatibility tests.
