...
`AgentLoop.run()` ([app/src/agent/agent-loop.ts](app/src/agent/agent-loop.ts)) uses two-phase scoping: the planner first opens one or more packs, then the first `skill_call` freezes those packs while leaving every skill inside them callable. A direct call without `open_packs` freezes the union of the packs represented by that call's validated `selectedSkills`. This replaced the earlier individual-skill freeze after production traces proved it blocked ordinary discovery chains such as `sheets_find` → `sheets_read`.
...
5. `open_packs` is **additive, not a replacement**, and may be issued more than once before the first `skill_call`: `openPacks = union(openPacks ?? [], decision.packs)`. For example, the planner can open `email`, realize it also needs `google`, and add it before execution. Once a real `skill_call` happens, the opened packs freeze and their complete registered skill sets remain callable.
6. `AgentLoop` tracks a loop-local `openPacks` alongside the existing `selectedSkills`. Summary selection for the prompt becomes three-tiered:
   - pack scope frozen → every registered skill in those packs is visible and callable.
...
7. Once the planner issues a real `skill_call`, the loop resolves the frozen packs back to concrete skill names for the executor. Policy, confirmation, and audit continue operating on the individual validated action and never treat a pack as blanket execution approval.
...
**Phase 4 progress (2026-08-22): `google` pack started ahead of schedule.** The user asked for generic, free-form Google Sheets capability before continuing the planned Phase 4 order, and explicitly chose to stop registering `record_expense`/`expense_report` in favor of managing expenses (and anything else) through free-form sheet skills instead — their code/tools/tests are untouched and `registerFinanceSkills` still works if this is ever reverted, it's just not called from `runtime.ts` anymore. New: `sheets_create` (new spreadsheet with formatted header + optional rows), `sheets_read` (arbitrary range read), `sheets_update` (arbitrary range write/append) — all reuse the existing `spreadsheets` OAuth scope, no new credentials needed. `finance` pack removed from the catalog (nothing registers into it now); `google` pack added. One open gap worth flagging: unlike the old binding/provisioning system, there's no durable "this is the expense sheet" pointer anymore — persisting which spreadsheet to reuse across turns now depends on Shiva's own memory system picking up the spreadsheet ID/URL from conversation, not a guarantee.

**`device` pack landed (2026-08-22), now 5 skills.** The Android companion app has real handlers for `device.contacts.search`, `device.phone.call`, `device.notifications.list`, `device.notifications.read`, and `device.camera.capture`; everything else it lists is prepared but unimplemented on the phone side and returns `UNSUPPORTED`. New `WS /device/ws` endpoint + `DeviceCommandDispatcher` (single-connection, correlates outbound `device_command` messages to inbound `device_command_result` replies by ID, times out cleanly if the phone never answers). Optional `DEVICE_WS_TOKEN` env var gates the connection — unset means no auth is enforced, matching the personal-instance trust model everything else here already assumes.

`device_camera_capture` needed real vision capability that didn't exist before: `ChatMessage` gained an optional `images` field (base64, no data: prefix), which flows straight through `OllamaProvider` to Ollama's `/api/chat` unchanged. The skill dispatches the capture, looks for the base64 image under a handful of plausible field names (the exact one the Android app uses isn't pinned down in the contract given so far), then makes a dedicated `provider.chat()` call asking the model to describe it — deliberately *not* done by having the planner's own decision call see the image, since that call is temperature-0 and JSON-schema-locked, not meant for open-ended vision prose. The raw base64 is never included in what's returned to the caller/audit log, only the resulting text description. If the image field isn't recognized, or the model can't describe it (not vision-capable, or the call fails), the skill still reports the capture as successful with a diagnostic note rather than failing outright — and that note lists the actual field names seen, so the real one can be added once known instead of guessed at.
...
- New tests in `agent-loop.test.ts`: additive multi-hop `open_packs`, pack-level `find` → `read` chaining, direct multi-pack freezing, rejection of `open_packs` after execution starts, invalid pack correction, and a prompt-budget regression proving the unscoped prompt cost scales with pack count rather than registered-skill count. New tests in `skill-registry.test.ts`: `PackRegistry` duplicate/malformed-name rejection, pack-existence enforcement only when supplied, and `listPacks()` grouping/aggregation.
...
- New `agent-loop` tests for the `open_packs` path: cold call → `open_packs` → optional second `open_packs` unions another pack → skills from both are visible → first `skill_call` freezes both packs while their complete skill sets remain usable.
