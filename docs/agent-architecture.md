...
Malformed planner JSON or a decision that does not match the decision schema is retried once within the same request deadline. An invalid initial called-skill/scope selection also receives one retry with the exact registered-name catalog. If either repair still fails before any tool has executed, the turn falls back to the normal streaming Shiva brain with an explicit instruction that no tool or current source was used. After any tool observation exists, planner/scope failures remain fail-closed; a free-form fallback may not reinterpret or claim an executed action.
